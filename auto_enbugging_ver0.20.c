...
void Enbug(char **prog,char **nrp,int line,int nob,struct RepTable *rt){
  ...
  for(count=trial=0;(count<abs(nob))&&(trial<MAXENBUGTRIAL);trial++){

    c=rand()%line;  // Choose random line
    j=rand()%(strlen(nrp[c])); // Choose random character
    mode=rand()%6;  // Choose Enbugging mode
    //mode=5;   //Test Mode
	...
	if ((j > 0 && nrp[c][j-1]=='=') || nrp[c][j+1]=='=' ) {
	  ...
	  */
	  ...
	  if (j > 0 && prog[c][j] == '=' &&
	    ...
	    if (nob < 0)
	      fprintf(stderr,"Bug #%04d : replace %c= into =%c in line %d (%d)\n",count+1,nrp[c][j-1],nrp[c][j-1],c+1,j-1);
  ...
  if (trial >= MAXENBUGTRIAL) {
    fprintf(stderr,"Sorry, the number of enbugging-trial becomes upper bound (%d)\n",MAXENBUGTRIAL);
  }
